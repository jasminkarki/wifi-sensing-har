...
directory=os.path.abspath("../csi_data")
# os.system("ls "+directory)

# Location where model parameter is stored
model_params =os.path.abspath("../model")
...
def dataframe_of_CSI(directory):
    ...
    df_walk.dropna(inplace=True)
    df_noact.dropna(inplace=True)
    df_jog.dropna(inplace=True)
...
if __name__ == "__main__":
    ...
    pickle.dump(pipe1, open("../model/pipefinal_svm.pkl","wb"))
    ...
    ## Dump Model to Target Location
    pickle.dump(pipe2, open("../model/pipefinal_knn.pkl","wb"))
